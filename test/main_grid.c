...
isz_grid_line_t *line_new(isz_orientation_t o, char *s, ISZ_FAIL_PARAM)
{
	...
	isz_id_i_t *isz_id_i = ISZ_OBJ_GET_I(line, isz_id);
	isz_it_t *id = isz_id_i->get(line, ISZ_FAIL);
	ISZ_FAIL_RET_CALL_IF_VAL(NULL);

	isz_str_owner_i_t *isz_str_owner_i = ISZ_IT_GET_I(id, isz_str_owner);
	isz_str_owner_i->look(ISZ_OBJ(id), s);
	return line;
}
isz_grid_field_t *field_new(char *s, ISZ_FAIL_PARAM)
{
	...
	isz_id_i_t *isz_id_i = ISZ_OBJ_GET_I(field, isz_id);
	isz_it_t *id = isz_id_i->get(field, ISZ_FAIL);
	ISZ_FAIL_RET_CALL_IF_VAL(NULL);
